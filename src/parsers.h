...
public:
	...
	SkipToNextLineParser* Clone() const { return new SkipToNextLineParser( *this ); }
};
...
public:
	...
	bool Parse( InStream& is ) 
	{
		...
		if( !is.good() ) return false;
		Char c = is.get();
		if( !IsBlank( c ) ) { is.unget(); return false; }
		while( is.good() && IsBlank( c ) ) c = is.get();
		if( is.good() ) is.unget(); 
		return true;
	}
...
public:
	...
	UIntParser( const UIntParser& p ) : name_( p.name_ ), token_( p.token_ ), valueMap_( p.valueMap_ ) {}
...
public:
	...
	IntParser( const IntParser& p ) : name_( p.name_ ), token_( p.token_ ), valueMap_( p.valueMap_ ) {}
...
private:
	...
	bool MatchUnsigned( InStream& is )
	{
		...
		if( !is.good() ) return false;
		if( ::IsDigit( c ) != 0 )
		{ token_.push_back( c ); Apply( &FloatParser::MatchUnsigned, is, 0 ); return true; }
		else if( c == '.' )
		{ token_.push_back( c ); return Apply( &FloatParser::MatchFractional, is, c ); }
		is.unget();
		return false;
	}
...
/// @brief Validates a constant string with support for case-insensitive validation.
/// @ingroup validators
...
public:
	...
	ConstStringValidator( const ConstStringValidator& csv )
		: value_( csv.value_ ), ignoreCase_( csv.ignoreCase_ ), count_( csv.count_ ) {}
...
private:
    ...
    /// Ignore case flag.
	bool ignoreCase_;
    /// Index pointing at the next character to validate in ConstStringValidator::value_.
...
public:
    ...
    /// @param name name associated to retrieved value(s); set as key in value map
    template < class T >
    SequenceParser( const T& v, const ValueID& name = ValueID() ) : validator_( v ), name_( name ) {} 
	/// Constructor.
    /// @param name name associated to retrieved value(s); set as key in value map 
    ...
    bool Parse( InStream& is )
	{
		...
		{
			c = is.get();
			if( !is.good() || !validator_.Validate( token_, c ) ) break;
			token_.push_back( c );
		}
		if( is.good() ) is.unget();
...
//------------------------------------------------------------------------------
/// @brief Convenience class that implements a parser for alphanumeric strings that
/// start with a letter.
///
...
public:
	...
	FirstAlphaNumParser* Clone() const { return new FirstAlphaNumParser( *this ); }	
private:
	String name_;
	String token_;
	mutable Values valueMap_;
...
//------------------------------------------------------------------------------
/// @brief Parser for sequences of values of the same type separated by a separator
...
public:
    ...
    /// @param b begin parser: parses expression identifying the start of the tuple
    ...
    /// @param blanks skip blanks flag: if @c true blanks are skipped before applying each parser
    TupleParser( const Parser& v, const ValueID& name = ValueID(), const Parser& b = BlankParser(),
		const Parser& s = BlankParser(), const Parser& e = BlankParser(), bool blanks = false ) : skipBlanks_( blanks ),
		beginParser_( b ), valueParser_( v ), separatorParser_( s ), endParser_( e ), name_( name ) {}
	TupleParser( const TupleParser& l ) 
		: skipBlanks_( l.skipBlanks_ ), name_( l.name_ ), beginParser_( l.beginParser_ ), 
		  valueParser_( l.valueParser_ ), separatorParser_( l.separatorParser_ ),
		endParser_( l.endParser_ ), values_( l.values_ ), valueMap_( l.valueMap_ )
	{}
    /// IParser::Parse implementation: applies begin parser, loops over values/separators then
	...
	bool Parse( InStream& is )
	{
        ...
        while( /*!( endReached = endParser_.Parse( is ) ) &&*/ valueParser_.Parse( is ) )
		{
			...
			// value & separator parser applied, if required skip blanks before next value or
            // end separator
            if( skipBlanks_ ) SkipBlanks( is );
		}
        ...
        // if end reached and number of parsed values is correct return true, false
        // otherwhise
		ok = endReached && ( counter == Size() || ( counter > 0 && Size() < 0 ) );
		}
		return ok;
	}
...
/// separator >= __ >= C( "|" ) ;
/// TupleParser<> tl4( 
///   NotParser<OrParser>( separator / NextLineParser() ), "strings", separator > __,
///   separator,
///   PassThruParser(),
///   SKIP_BLANKS ); // end parser always succeeds i.e. no end parser separator for tuple
...
public:
	...
	const ValueType& operator[]( const KeyType&  ) const { static const ValueType v; return v; }
